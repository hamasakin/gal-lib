...
Last activity: 2026-07-09 — Quick 260709-mnc：补齐捆绑 LE 缺失的 LECommonLibrary.dll 修复转区启动崩溃 + 接入 tauri-plugin-single-instance 单实例守卫
...
| 260526-2f9 | Cargo `[profile.dev-release]` 本地快编 profile —— 继承 release 但 `codegen-units=16` / `lto="thin"` / `incremental=true` / `opt-level=2`；本地 `pnpm tauri build --no-bundle -- --profile dev-release` 实测 108 秒（cargo 自报 1m39s），相比发版 release（用户感知 ~10 分钟）约 5x 提速；产出 `target/dev-release/gal-lib.exe` 15.2 MB（release 8.0 MB）；**未触碰 `[profile.release]`，`npm run release` 行为 100% 不变**；Tauri 2 CLI 不吃 `--profile`，须用 `--` 转给 cargo（首次尝试 `pnpm tauri build --profile dev-release --no-bundle` 报 `unexpected argument '--profile'`，已在 PLAN/SUMMARY 标注）；6 个 pre-existing 死代码 warning 与本任务无关 | 2026-05-26 | 80209a0 | [260526-2f9-cargo-dev-release-profile](./quick/260526-2f9-cargo-dev-release-profile/) |
| 260531-x57 | 添加单个游戏自动匹配元数据 + 拖拽添加游戏 —— **核实结论：需求1（添加单个游戏自动抓元数据）此前已完整实现**，Settings.tsx 已有 `onAddSingleGame`/`single-add` Section 调 `addGame`→后端 `add_game`→`ingest_one_dir`（Bangumi/VNDB 抓取 + 封面缓存 + enrichment），本次零改动。需求2（拖拽添加）本次新实现：App.tsx 加 `getCurrentWebview().onDragDropEvent` 监听（race-safe 清理）+ drop overlay（pointer-events-none 深色遮罩）+ 把拖入每个路径串行交给同一 `addGame`（后端非目录路径返 Err、按 Ok/Err 计数，零新依赖不引 plugin-fs）+ 汇总 toast；tauri.conf.json window 显式 `dragDropEnabled: true`；i18n 三语新增 `dragdrop.*` 4 key → 527/527/527 对齐。验证：pnpm tsc --noEmit EXIT=0 / pnpm build EXIT=0（1988 modules）/ 后端 Rust 零改动（git diff src-tauri/src 空）。GUI 拖放真机验证 7 项（Task1 三项 + Task2 四项）+ 2 处假设（onDragDropEvent payload 字面值、sonner toast.warning 首用）待用户确认 | 2026-05-31 | d57857d | [260531-x57-add-metadata-drag-drop](./quick/260531-x57-add-metadata-drag-drop/) |
| 260603-2g0 | 拖拽添加游戏实时刷新列表 —— 此前 drop 循环 `for(p) await addGame(p)` 串行等每个游戏跑完元数据抓取再进下一个、整批结束才刷新一次，拖入后长时间无反应。本次接上项目现成实时管线：新增批量后端命令 `add_games(paths)` + `AddGamesResult{added,failed}`，两阶段——Phase1 先把所有路径 `insert_placeholder_dir` 并每条 `emit("games-changed")`（占位卡瞬间全出带「获取中」徽标），Phase2 逐个 `enrich_metadata_for_dir` 前后发 `meta-fetch-progress` started/finished + `games-changed`（卡片渐进回填封面/标题 + 脉冲高亮），enrich 失败仅 eprintln 不回退占位行、不减 added；lib.rs 注册 `add_games`。前端 App.tsx drop 改单次 `await addGames(paths)`，落点立即 `toast.loading(dragdrop.adding)` 结束转 success/partial，保留兜底 `searchGames` 刷新（覆盖落点不在 Library 路由场景）；scan.ts 加 `addGames` 包装 + 类型；i18n 三语加 `dragdrop.adding`。复用 Library.tsx 既有 `games-changed` 节流订阅 + main.tsx `meta-fetch-progress` 订阅，前端几乎零额外接线。验证：cargo check 0 error / pnpm typecheck EXIT=0（cargo build 仅最后替换 gal-lib.exe「拒绝访问」因 dev app 运行锁住 exe，非代码问题）。GUI 真机验证 3 项（多目录拖入占位卡瞬间全出+逐个回填 / 落点 loading toast→已添加 N 个 / 含无效路径 partial 文案）待用户确认 | 2026-06-03 | d61cf08 | [260603-2g0-realtime-drag-add](./quick/260603-2g0-realtime-drag-add/) |
...
| 260603-3w9 | 自动检测更新改为"先弹窗提醒、用户确认才下载安装"，不再静默强制下载 —— updater.ts 拆分检查与下载（UpdaterState 新增 available 分支携带 Tauri Update 句柄、checkForUpdates 改为仅检查命中返 available 不再自动下载、新增 downloadAndInstallUpdate(update) 由调用方确认后触发）；App.tsx 删掉启动 5s setTimeout 改为挂载即检查（保留 cancelled 守卫），命中弹「发现新版本」toast 带「现在更新」action、点了才下载、完成换「立即重启」toast；AboutSection.tsx 手动检查停在 available 态显示「下载更新」按钮、点击走下载→ready→重启；三语 translation.json 新增 6 个 key（toast.update_available/_desc/update_now/update_downloading、about.update_available/download_update）并修订 about.auto_check_desc 去掉「5秒后台下载」旧语义。验证：pnpm exec tsc --noEmit 全绿 / verify-i18n.cjs ok / grep 核对 downloadAndInstall 仅在 downloadAndInstallUpdate 内、App.tsx 已无 5000。GUI 真机验证（命中新版本弹提示不自动装 / 点更新才下载 / 手动检查停 available / 三语）待人工确认 | 2026-06-03 | b227c8d · 3a799d7 · e779e24 | [260603-3w9-update-prompt-dialog](./quick/260603-3w9-update-prompt-dialog/) |
