...
last_activity: 2026-05-09 — Completed quick task 20260509g: scan cancel checkpoints (start_scan placeholder/ingest loops + refresh_all_metadata loop share ScanState ctx) + cross-game ingest concurrency (JoinSet, INGEST_CONCURRENCY=4) + per-scan QueryCache (OnceCell anti-thundering-herd) + GameGrid useVirtualizer row-mode (ResizeObserver-driven columnCount, scroll container reused from Library.tsx flex-1 wrapper)
progress:
  total_phases: 5
  completed_phases: 1
  total_plans: 1
...
- **20260509f**: 2-phase ingest split lives at the **command layer** (commands.rs::insert_placeholder_dir + enrich_metadata_for_dir), NOT in ingest.rs — keeps `process_game` / `refresh_for_query` signatures untouched so add_game / refresh_metadata / bind_metadata / refresh_all_metadata don't need rewiring. `meta-fetch-progress` payload uses `serde_json::json!({ game_id, phase })` literal (no Serialize struct — fields trivial, emit points scattered across 4 commands). bind_metadata + refresh_metadata wrap inner work in `let result: Result<...> = async { ... }.await` so the `finished` emit ALWAYS fires on both Ok and Err paths (no scopeguard dep). Frontend `fetchingMetaIds: Record<number, true>` (NOT Set) + per-card boolean selector `s.fetchingMetaIds[game.id] === true` so each card only re-renders on its own id transitions. `removeFetchingMetaId` + `clearFetchingMetaIds` short-circuit when no-op to avoid spurious reference bumps. Pulse-ring CSS animates **only opacity** (box-shadow held constant via selector) — GPU-friendly, stacks cleanly with `-translate-y-1` hover transform. Orphan placeholder cleanup `DELETE FROM games WHERE metadata_source IS NULL AND last_scanned_at IS NULL` runs at lib.rs::setup tail (after close-to-tray window-event handler, before Ok(())) via `tauri::async_runtime::spawn` — safe because in-progress scans belong to dead prior process; lives in setup not migrations (data ≠ schema). add_game intentionally has NO emit (no card-on-screen-while-fetching scenario for single-add path). first-event refetch deliberately skipped — placeholders surface via existing scan-progress completed→Library refetch chain (CONTEXT compromise accepted to keep event surface minimal).
...
| 20260509f | realtime meta-fetch card status: backend `meta-fetch-progress { game_id, phase: started\|finished }` emitted from start_scan / refresh_metadata / bind_metadata / refresh_all_metadata (2-phase ingest split: insert_placeholder_dir + enrich_metadata_for_dir; bind/refresh wrap inner work in `async{}.await` so finished emit fires on error path); zustand `fetchingMetaIds: Record<number,true>` slice + main.tsx module-scope listener + scan-progress terminal status `clearFetchingMetaIds()` fallback; GameCard cover gains `.pulse-ring` (1.6s opacity 1↔0.5) + Loader2 「获取中」 badge with `var(--accent)` color when isFetchingMeta; setup-time `DELETE FROM games WHERE metadata_source IS NULL AND last_scanned_at IS NULL` purges orphan placeholders left by crashed prior-process scans | 2026-05-09 | 127a524 / 485087c / cb99127 | [20260509f-realtime-meta-fetch-card-status](./quick/20260509f-realtime-meta-fetch-card-status/) |
| 20260509g | scan cancel + cross-game concurrency + grid virtualization: (1) **cancel fix** — start_scan 占位 INSERT 循环 / ingest JoinSet 每个并发 task 顶部 / refresh_all_metadata 循环顶部都加 `ctx.cancel.load(Relaxed)` 检查，命中 emit `ScanStatus::Cancelled` 立即 return；refresh_all_metadata 进入 spawn 前 `scan_state.ctx = Some(Arc::new(ScanContext::new()))` 与 start_scan 共享同一 ScanState slot 让 cancel_scan 命令同时能取消两条路径。(2) **跨 game 并发 ingest** — start_scan ingest 改为 `tokio::task::JoinSet`，常量 `INGEST_CONCURRENCY=4`；scan-progress.completed 从 `enumerate.i+1` 改为 `Arc<AtomicUsize>` + `fetch_add(1, Relaxed)` 单调递增；同 game 内 Bangumi+VNDB tokio::join! 并发查 + aggressive_candidates sequential fan-out 全部保持现状（用户决策：同 game 不并发）。(3) **跨 game query dedup** — `ingest::QueryCache = Arc<Mutex<HashMap<String, Arc<tokio::sync::OnceCell<Option<Candidate>>>>>>`，scope = 单次 start_scan 调用；`pick_best_with_cache(query, &cache)` 用 `OnceCell::get_or_init` 让多个 task 命中同一 cleaned query 时只发一次 Bangumi+VNDB 搜索（防 thundering-herd），其他 caller await 同一 future；新增 `process_game_cached` 平行函数（`process_game` / `refresh_for_query` 签名不动 — add_game / refresh_metadata / bind_metadata / refresh_all_metadata 调用路径完全保持原样，cache 只在 start_scan 路径注入）。(4) **GameGrid 重启虚拟化** — 用 `@tanstack/react-virtual` 的 `useVirtualizer` row-mode + manual lane indexing；ResizeObserver 测 scrollContainer width 推 `columnCount = floor((w - padding*2) / (cardW + gap))`；scrollContainer 复用 Library.tsx 的 `<div flex min-h-0 flex-1 flex-col overflow-auto>`（通过 `scrollContainerRef: RefObject<HTMLDivElement\|null>` prop 下放给 useVirtualizer 的 `getScrollElement`），GameGrid 内部不再套 overflow-auto；DensityToggle 切换 `--card-w` 时 ResizeObserver 自动触发 columnCount 重算 + 虚拟化器 reflow；overscan=5 行 | 2026-05-09 | de4ac19 / 5c1d830 / fa2b7aa | [20260509g-fix-cancel-concurrent-ingest-virtualize](./quick/20260509g-fix-cancel-concurrent-ingest-virtualize/) |
