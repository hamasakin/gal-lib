...
last_updated: "2026-05-25T13:55:00.000Z"
last_activity: "2026-05-25 — Quick 260525-tw2：修复 v13 migration 漏注册导致 search_games 报错、游戏列表全空。上一轮 quick 260525-g1m 生成了 0013_add_external_rating.sql 但漏在 db.rs::migrations() vec 注册 v13，旧库 schema 卡在 v12 缺三列；search_games SELECT external_rating 触发 Err，Library.tsx:340-343 前端 catch 只 console.error 不清空 store.games，结果用户看到「侧边栏分类条目数正常但游戏列表全空」。修复：src-tauri/src/db.rs 单文件 3 处定向追加（module-doc 末段 v13 段 + const V13_SQL include_str! + Migration entry version:13），参照 v11/v12 写法。cargo check 通过；GUI 真机验证待用户重启 v0.3.2 build 确认列表恢复（migration 自动从 v12 升 v13、external_rating 列被创建、SELECT 不再 Err）。后续动作：验证 OK 后由主对话 npm run release patch → v0.3.2。"
progress:
  total_phases: 4
  completed_phases: 4
  total_plans: 16
