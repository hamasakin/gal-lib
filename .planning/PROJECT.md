...
Run `/gsd-new-milestone` to formalise. Likely scope (carry from v1.1+v1.2):

- v1.1 carry: standalone `/scan` route + KPI strip + Bangumi/VNDB review queue (PGE-01/02); `tauri-plugin-opener` open-directory / open-screenshots-dir; Detail `?tab=` deeplink parsing; LIB-02 reinstate-or-drop; real session count IPC
- v1.2 carry: cross-source person dedup; persons aggregate page enrichment (作品时光轴 + 同台伙伴推荐 — seed); person portrait local caching; backfill progress UI 完整化
- v1.2 real-app smoke: UI-01/02/03 real-app verification
...
(v1.3 backlog — to be formalised via `/gsd-new-milestone`)

- [ ] 标准 `/scan` 路由 + 顶部 KPI 4 联 + 双栏 feed + 待复核卡片队列（PGE-01 carry-over from v1.1）
- [ ] Bangumi/VNDB 候选并排对比 + 一键切换数据源（PGE-02 carry-over from v1.1）
...
- [ ] 跨源人物去重（v1.2 carry — 同一作家 Bangumi+VNDB 双源各占一行）
- [ ] 人物聚合页加强（作品时光轴 + 同台伙伴推荐，v1.2 seed）
- [ ] 人物头像本地缓存（v1.2 carry）
- [ ] Backfill 进度 UI 完整化（v1.2 carry — 事件已 emit，PageHeader 进度条待补）
...
---
*Last updated: 2026-05-09 after v1.2 milestone (Metadata Enrichment & Filtering) shipped*
