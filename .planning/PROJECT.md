...
**Last shipped:** v1.3 Scan Pipeline & Person Polish (2026-05-12)
**Status:** Functional MVP + 「图书馆」 visual identity + 元数据深度增强 + 独立扫描复核流 + 人物聚合页二阶 + 文件系统打磨
**Stack:** Tauri 2 + Rust + React 19 + Vite + TypeScript + Tailwind v3 + shadcn + Zustand + SQLite (rusqlite); schema v9
**Bundle:** ~730 KB JS (gzip 219 KB) + 58 KB CSS (post-v1.3 measurement, 2026-05-12)
**Routes:** `/` (Library), `/games/:id` (Detail), `/persons/:id` (人物聚合), `/scan` (扫描复核), `/stats`, `/screenshots`, `/settings`

v1.3 closed with 17/17 requirements code-shipped + 16 plans across 4 phases (12-15). All automation green: 68/68 cargo tests + pnpm tsc + pnpm build. Backend additions: `scan_review_queue` table + 5 scan IPCs (get_scan_kpis / list_scan_review_queue / dismiss_review_item / accept_review_candidate / fetch_review_candidates) + 4 person/backfill IPCs (list_co_staff_for_person / get_or_fetch_portrait / cancel_backfill / get_session_count) + 1 filesystem IPC (open_path via tauri-plugin-opener). Frontend additions: `/scan` route + ReviewQueue + ScanFeed + PersonTimeline + CoStaffStrip + BackfillProgressBar + Detail controlled Tabs (?tab= deeplink). 12-step real-app walkthrough (V-01..V-12) carry to v1.4 milestone audit per autonomous-mode policy.

## Next Milestone

_None defined — v1.3 just shipped. Run `/gsd-new-milestone` to define v1.4. Highest-priority v1.4 carry-over: execute walkthrough V-01..V-12 on real Win10/11 with Locale Emulator + populated library._
...
**v1.3 Scan Pipeline & Person Polish (2026-05-12):**
- ✓ Schema v9：scan_review_queue 表 — v1.3
- ✓ `/scan` 独立路由 + KPI 4 联 + 双栏 ScanFeed/ReviewQueue + Bangumi/VNDB 候选并排对比一键采用 — v1.3 (real-app smoke → v1.4)
- ✓ 跨源人物去重（merge_persons 在 IPC 查询层折叠 Bangumi+VNDB；sources/person_ids attribution）— v1.3
- ✓ 人物聚合页时光轴（PersonTimeline 横向年份气泡 + playtime 映射） — v1.3
- ✓ 「常与 X 共同出现」横滑条（list_co_staff_for_person + role_hint 子查询） — v1.3
- ✓ 人物头像本地缓存（portrait_cache.rs Bangumi /v0/persons + data/portraits/） — v1.3 (VNDB → v1.4)
- ✓ Backfill 进度条 + 取消按钮（BackfillProgressBar + cancel_backfill IPC + AtomicBool） — v1.3
- ✓ `tauri-plugin-opener` 集成 + open_path IPC + GameCard/Screenshots 「打开目录」入口 — v1.3
- ✓ Detail `?tab=` deeplink 受控 Tabs（DETAIL_TABS 6 个值 + useSearchParams） — v1.3
- ✓ Stats 真实会话数 KPI（get_session_count IPC SELECT COUNT(*) FROM sessions WHERE ended_at IS NOT NULL） — v1.3
- ✓ LIB-02 杂志式不对称网格 **废止决策** 写入 Key Decisions — v1.3

### Active

<!-- No active milestone. Run /gsd-new-milestone to define v1.4. -->

_v1.3 just shipped. v1.4 not yet defined. Highest-priority carry-over: execute 12-step walkthrough V-01..V-12 in `.planning/milestones/v1.3-phases/15-v12-real-app-smoke/15-SUMMARY.md`._
...
**v1.3 后状态**：
- 视觉系统 v1.1 完整保留，元数据层 v1.2 完成，v1.3 一次性清完 v1.1/v1.2 carry-over
- Rust IPC 表面扩展（v1.0–v1.2 全部保留 + 新增 10：get_scan_kpis / list_scan_review_queue / dismiss_review_item / accept_review_candidate / fetch_review_candidates / list_co_staff_for_person / get_or_fetch_portrait / cancel_backfill / get_session_count / open_path）
- Schema v9：games + sessions + tags + game_tags + scan_roots + screenshots + save_backups + persons + game_staff + game_official_tags + custom_views + custom_view_games + scan_review_queue
- 文件系统：tauri-plugin-opener 接入；data/portraits/ 新增（per-person Bangumi portrait cache）
- 已知待办：12-step real-app walkthrough V-01..V-12（v1.4 第一件事，需 Win10/11 + LE + 真实库）+ VNDB portrait + 跨源 persons 物理合并 + co-staff 权重升级 + 5 个 pre-existing cargo warnings 清理
